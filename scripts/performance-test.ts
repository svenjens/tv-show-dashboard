...
async function measureRequest(url: string, description: string): Promise<TestResult> {
...
function printResult(result: TestResult, baseline?: number) {
  ...
  const size = result.size
    ? ` ${COLORS.gray}[${(result.size / 1024).toFixed(1)}KB]${COLORS.reset}`
    : ''
...
async function runPerformanceTest(showId: number, baseUrl: string) {
  ...
  if (fullLoad.success && fastLoad.success) {
    const improvement = (
      ((fullLoad.duration - fastLoad.duration) / fullLoad.duration) *
      100
    ).toFixed(1)
    ...
    console.log(
      `  Time saved per page load: ${COLORS.cyan}${fullLoad.duration - fastLoad.duration}ms${COLORS.reset}`
    )
  ...
  if (episodesFull.success && episodesFast.success) {
    const episodeImprovement = (
      ((episodesFull.duration - episodesFast.duration) / episodesFull.duration) *
      100
    ).toFixed(1)
